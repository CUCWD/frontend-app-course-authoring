...
import {
  bool, func, string, arrayOf, shape, object, instanceOf,
} from 'prop-types';
...
function ImportedData({ keyTerm }) {
  ...
  return (
    <div>
      ...
      <p>
        <ul>
          {definitions.map((descr) => <li>{descr.description}</li>)}
        </ul>
      ...
      <div className="flex-row">
        ...
        <div className="modal-col-right flex-row">
          <p>
            ...
            <ul>
              {oldDefinitions.map((descr) => <li>{descr.description}</li>)}
            </ul>
          </p>
        </div>
      </div>
    ...
    for (let e = 0; e < termData.length; e++) {
      if (fileData[i].key_name === termData[e].key_name) {
        ...
        if (flagTermEdit.definitions.length > termData[e].definitions.length) {
          for (
            let k = termData[e].definitions.length;
        ...
        <ModalDialog.Body>
          ...
          >
            {fileData
              ...
              .map((keyTerm) => <ImportedData key={keyTerm.id} keyTerm={keyTerm} />)}
          </Collapsible>
        </ModalDialog.Body>
